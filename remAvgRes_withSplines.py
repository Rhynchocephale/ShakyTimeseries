...
def makeAllTrusted( currentFolder, fileName ):
...
def splines(enu,pos,dateList,currentFolder):
    ...
    for i in range(0,lenDateList): #for each day
        ...
        for j in [0,1,2]: #for each dimension
            ...
            #if True:
                spli = np.asarray(spl.interp_spl(X,Y,Zalti,Zinterp,10^2))
    ...
    if not os.path.exists(currentFolder+'/splines/'): #creates the /enu directory if it does not already exists
        os.makedirs(currentFolder+'/splines/')
        
    for i in range(0,lenDateList):
        np.savetxt(currentFolder+'/splines/'+str(time.time())+'.txt',spl_param[:,i,:])
    
    return [spl_param,nbOfUsedStations]

def corrSpline(enu,staCount,position,dist,spl,day):
    enu_corr = np.zeros(3)
...
def remAvgRes( currentFolder, trustedList ):
    
    #currentFolder = os.path.dirname(os.path.abspath(__file__))
    f = open(trustedList,'r')
    trusted = f.readline().split()
    trusted.sort()                          #returns list of names of trusted stations
    f.close()
                ...
                for j in range(max(r-nbDaysCalc,0),min(r+nbDaysCalc,posPerDay.shape[0])):  #makes sure not to get out of the matrix, takes nbDaysCalc lines after and nbDaysCalc before, when possible
                    if j!=r:
                        ...
                        if any(posPerDayLine):    #if not full of zeros
                            ...
                            k += 1
                            listOfObs = np.vstack((listOfObs,posPerDayLine))
                    
                if k>3:
                    tmpsum /= float(k)
                    jumpPos = -1
                    if any(abs(row-tmpsum)[0:2] > 10): #if more than 1cm difference between pos and avg
                        #OK, COMPLICATED SHIT OVER HERE
                        jumpPos2 = r
                        tmpmax = 0
                        while jumpPos2 != jumpPos: #while not the biggest jump
                            jumpPos = jumpPos2
                            for u in range(max(jumpPos-nbDaysCalc,0),min(jumpPos+nbDaysCalc,posPerDay.shape[0])):  #maybe a bigger jump around?
                                if any(posPerDay[u,:]):
                                    tmpsum2 = 0
                                    k2 = 0
                                    listOfObs2 = np.zeros((1,3))
                                    for v in range(max(u-nbDaysCalc,0),min(u+nbDaysCalc,posPerDay.shape[0])): #looking for jump
                                        if v!=u:
                                            posPerDayLine = posPerDay[v,:]
                                            if any(posPerDayLine):    #if not full of zeros
                                                tmpsum2 += posPerDayLine
                                                k2 += 1
                                                listOfObs2 = np.vstack((listOfObs2,posPerDayLine))
                                
                                    if k2 > 3:
                                        tmpsum2 /= float(k2)
                                        umax = max(abs(posPerDay[u,0:2]-tmpsum2[0:2])) #is this day the biggest jump so far?
                                
                                        if umax > tmpmax:
                                            tmpmax = umax
                                            #tmpsumJumpPos = tmpsum2
                                            listOfObs2 = np.delete(listOfObs2, (0), axis=0)
                                            tmpsumJumpPos = np.median(listOfObs2,axis=0)
                                            jumpPos2 = u
                        
                        
                        enu[nbSta,jumpPos2,0:3] = posPerDay[jumpPos2,:]-tmpsumJumpPos
                        enu[nbSta,jumpPos2,3] = 1
                        #print(enu[nbSta,jumpPos2,0:3])
                        #print(jumpPos)
                        posPerDay[jumpPos2,:] = [0,0,0]
                        r-=nbDaysCalc+1
                        currentDate -= datetime.timedelta(days=nbDaysCalc+1)
                    else:
                        if (enu[nbSta,r,3] == 0): #if has not been a faraway point, or if we have moved backward recently
                            #tmpsum = (tmpsum*k+row)/(k+1)
                            listOfObs = np.delete(listOfObs, (0), axis=0)
                            tmpsum = np.median(listOfObs,axis=0)
...
#------------------------------------------------------------------
    ...
    u=0
    #sd=open(currentFolder+'/000.txt','w')
    staCount = 0
    for sta in fileList:
        ...
        g.write('name\tepoch\tx\ty\tz\tstations_for_smoothing\n')
        errors = 0
        for line in f:            
            line = line.split()
            ...
            if any(xyz):                   
                if i>=0 and i<deltaT:
                    ...
                    if (enu[staCount,i,3] == 0):
                        ...
                        xyz_corr,more_errors = corrSpline(enu,staCount,position,dist,spl_param,i)
                        errors += more_errors
                        #sd.write(str(more_errors)+'\n')
                        ...
                        if any(xyz_diff > 30): #if correction > 3cm
                            print(xyz)
                            print(xyz_corr)
                            xyz2 = np.array(xyz)
                            #- np.array(enu[staCount,i,0:3])
                            print("too big correction:")
                            print(xyz_diff)
                            print('====')
                            
                        g.write(line[0]+'\t'+line[1]+'\t'+"{:.4f}".format(xyz2[0])+'\t'+"{:.4f}".format(xyz2[1])+'\t'+"{:.4f}".format(xyz2[2])+'\t'+"{:.0f}".format(nbOfStationsUsedToday)+'\n')
                    #else:
