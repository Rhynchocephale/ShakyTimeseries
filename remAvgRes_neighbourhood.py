...
def makeAllTrusted( currentFolder, fileName ):
    ...
    staList = []
    for f in os.listdir(currentFolder + '/mercator'):
        if os.path.isfile(os.path.join(currentFolder + '/mercator',f)) and f.endswith('.dat'):
...
def getDates(currentFolder,trusted):
    ...
    for sta in trusted:
        staFile = currentFolder + '/mercator/' + sta + '_igs08_enu.dat'    #gets to the file
        f = open(staFile,'r')
...
def remAvgRes_nb( currentFolder, trustedList ):
...
#------------------------------------------------------------------
    ...
    for sta in fileList:
        ...
        absSta = currentFolder+'/mercator/'+sta
