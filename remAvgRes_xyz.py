...
def remAvgRes( trustedList ):
    ...
    for sta in trusted:  #for each trusted station                                   
        ...
        r=0
        currentDate = minStart
        nbDaysCalc = 10 #nb of days taken into account for running avg (*2+1)
        while r < posPerDay.shape[0]:       #reads the new matrix line by line to compute avg
            ...
            if any(row) and r>beginDate:    #if not full of zeros and after beginning date
                ...
                if k>3:
                    tmpsum /= float(k)
                    jumpPos = -1
                    if any(abs(row-tmpsum)[0:3] > 0.01): #if more than 1cm difference between pos and avg
                        ...
                        while jumpPos2 != jumpPos: #while not the biggest jump
                            ...
                            for u in range(max(jumpPos-nbDaysCalc,0),min(jumpPos+nbDaysCalc,posPerDay.shape[0])):  #maybe a bigger jump around?
                                if any(posPerDay[u,:]):
                                    ...
                                    if k2 > 3:
                                        tmpsum2 /= float(k2)
                                        umax = max(abs(posPerDay[u,:]-tmpsum2)) #is this day the biggest jump so far?
